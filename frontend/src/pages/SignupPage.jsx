...
const SignupPage = () => {
  ...
  const [usernameUniqueStatus, setUsernameUniqueStatus] = useState('idle'); // 'idle' | 'checking' | 'available' | 'taken'
  const [emailUniqueStatus, setEmailUniqueStatus] = useState('idle'); // 'idle' | 'checking' | 'available' | 'taken'
  ...
  const handleSubmit = async (e) => {
    ...
    // --- Frontend Validations (Matching Admin Dashboard) ---
    ...
    // 2. Email domain: must end with @gmail.com
    ...
    // 3. Password strength: at least 8 chars, 1 uppercase, 1 lowercase, 1 number, 1 special char
    ...
    // 5. DOB: must be older than 16 years old based on today's registration date
    ...
    try {
      // Create user payload (excluding confirmPassword)
      const payload = {
        ...
        created_at: regDate.toISOString() // Transmit client machine registration date/time
      };
        ...
        <section className="flex-1 flex items-center justify-center p-24 bg-[#F8F9FB] overflow-y-auto">
          <div className="w-full max-w-[512px] flex flex-col gap-8 pb-4 mt-20">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-[20px]">
              ...
              {/* Organization (Plan dropdown removed/defaulted to Standard) */}
              <div className="flex flex-col gap-[6px]">
                <label className="font-body text-xs leading-4 tracking-[1.2px] uppercase text-[#44474D]">
                  Organization
                </label>
